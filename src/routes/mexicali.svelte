<script>
  ...
  const title = "Garita Center Reporte Garitas Mexicali Línea";
  const description =
    "Las Garitas de Mexicali se encuentran abiertas. Conoce el Reporte de Garitas y entérate como esta la línea para cruzar por Mexicali.";

  function printPort(value) {
    if (value === "west") {
      return "Mexicali Centro - Calexico West";
    }

    return "Nuevo Mexicali - Calexico East";
  }

  onMount(async () => {
    report = await getReportFor("mexicali");
    console.log(report);
  });
</script>
...
<section>
  <div class="msg">
    Las Garitas de Mexicali están abiertas.
    <br />
    <a href="./faq">Conoces los requerimientos</a>.
  </div>
  {#if report}
    {#each report as port}
      <h2>{printPort(port.port)} <small>Reporte de Garitas</small></h2>
      <ul>
        {#if port.data}
